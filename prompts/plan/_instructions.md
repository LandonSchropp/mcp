...
1. If the scope of the {{planType}} is not clear from the prompt's arguments or the previous context, ask the user to: "Please describe the {{planType}} or provide a Linear issue ID or URL." If the user responds with a Linear issue, fetch its details using the Linear MCP server.
